const Responsable_eglise = require('../models/Responsable_eglise');
const fichierImageModifier = require('fs');
const web_token = require('jsonwebtoken');

exports.ajoute_Responsable_eglise = (req, res, next) => {
   ...
   console.log(req.body.description);
    ...
    console.log(data);
    const responsable_eglise = new Responsable_eglise({
      ...data,
      file:`${req.protocol}://${req.get("host")}/images/${req.file.filename}`,
    });
    responsable_eglise.save()
      .then(() => res.status(201).json({ message: "objet sauces ajouter avec succe!" }))
      .catch((error) => res.status(400).json({ error }));
  };

  exports.Afficher_Responsable_eglise = (req, res, next)=>{
    console.log("Responsable_eglise");
    Responsable_eglise.find()
      ...
      //supprimer
exports.delete_Responsable_eglise = (req, res, next) => {
    console.log("suppression Responsable_eglise.....");
    Responsable_eglise.findOne({ _id: req.params.id })
    .then((data) => {
     ...
     // if (data.userId === req.auth.userId || req.auth.role === true) {
        ...
        Responsable_eglise.deleteOne({ _id: id })
          .then((data) => res.status(200).json(data))
          .catch(error => res.status(400).json({ error }));
...
exports.Recherche_pour_modifier_Responsable_eglise = (req, res, next) => {
    const id = req.params.id
    Responsable_eglise.findOne({ _id: id })
      .then((data) => res.status(200).json(data))
      .catch(error => res.status(400).json({ error }));
  }
...
//modification
exports.modifier_responsable = (req, res, next) => {
    ...
    Responsable_eglise.findOne({ _id: req.params.id })
      .then((data) => {
       ...
       // if (data.userId === req.auth.userId || req.auth.role === true) {
          ...
          Responsable_eglise.updateOne({ _id: req.params.id }, { ...ObjetNouveaufichier, _id: req.params.id })
            .then((data) => res.status(200).json(data))
            .catch(error => res.status(400).json({ error }));
       // }
      })
